    ...
    def call(self, inputs, **kwargs):
        ...
        try:
            ...
            resized = tf.image.resize_images(
                inputs, [new_height, new_width], align_corners=True
            )
        return resized
...
def residual_conv(prev, level, pad=1, lvl=1, sub_lvl=1, modify_stride=False):
    ...
    if modify_stride is False:
        prev = Conv2D(
            64 * level, (1, 1), strides=(1, 1), name=names[0], use_bias=False
        )(prev)
    elif modify_stride is True:
        prev = Conv2D(
            64 * level, (1, 1), strides=(2, 2), name=names[0], use_bias=False
        )(prev)
    ...
    prev = Activation("relu")(prev)
    prev = Conv2D(
        256 * level, (1, 1), strides=(1, 1), name=names[4], use_bias=False
    )(prev)
    prev = BN(name=names[5])(prev)
    return prev


def short_convolution_branch(
    prev, level, lvl=1, sub_lvl=1, modify_stride=False
):
    lvl = str(lvl)
    sub_lvl = str(sub_lvl)
    names = [
    ...
    if modify_stride is False:
        prev = Conv2D(
            256 * level, (1, 1), strides=(1, 1), name=names[0], use_bias=False
        )(prev)
    elif modify_stride is True:
        prev = Conv2D(
            256 * level, (1, 1), strides=(2, 2), name=names[0], use_bias=False
        )(prev)
...
def residual_short(
    prev_layer, level, pad=1, lvl=1, sub_lvl=1, modify_stride=False
):
    prev_layer = Activation("relu")(prev_layer)
    block_1 = residual_conv(
        prev_layer,
        level,
        pad=pad,
        lvl=lvl,
    ...
    block_2 = short_convolution_branch(
        prev_layer, level, lvl=lvl, sub_lvl=sub_lvl, modify_stride=modify_stride
    )
...
def residual_empty(prev_layer, level, pad=1, lvl=1, sub_lvl=1):
    ...
    block_1 = residual_conv(
        prev_layer, level, pad=pad, lvl=lvl, sub_lvl=sub_lvl
    )
...
def ResNet(inp, layers):
    ...
    res = MaxPooling2D(pool_size=(3, 3), padding="same", strides=(2, 2))(
        relu1
    )  # "pool1_3x3_s2"
...
def interp_block(prev_layer, level, feature_map_shape, input_shape):
    if input_shape == (473, 473):
        ...
        print(
            "Pooling parameters for input shape ",
            input_shape,
            " are not defined.",
        )
        exit(1)
    ...
    prev_layer = Conv2D(
        512, (1, 1), strides=(1, 1), name=names[0], use_bias=False
    )(prev_layer)
...
def build_pyramid_pooling_module(res, input_shape):
    ...
    feature_map_size = tuple(
        int(ceil(input_dim / 8.0)) for input_dim in input_shape
    )
    ...
    res = Concatenate()(
        [res, interp_block6, interp_block3, interp_block2, interp_block1]
    )
    return res


def _build_pspnet(
    nb_classes, resnet_layers, input_shape, activation="softmax", channels=3
):
