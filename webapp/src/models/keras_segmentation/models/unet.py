...
def unet_mini(n_classes, input_height=360, input_width=480, channels=3):
    ...
    up1 = np.concatenate(
        [UpSampling2D((2, 2), data_format=IMAGE_ORDERING)(conv3), conv2],
        axis=MERGE_AXIS,
    )
    conv4 = Conv2D(
        64,
    ...
    up2 = np.concatenate(
        [UpSampling2D((2, 2), data_format=IMAGE_ORDERING)(conv4), conv1],
        axis=MERGE_AXIS,
    )
    conv5 = Conv2D(
        32,
    ...
    o = Conv2D(n_classes, (1, 1), data_format=IMAGE_ORDERING, padding="same")(
        conv5
    )
...
def _unet(
    ...
    o = np.concatenate([o, f3], axis=MERGE_AXIS)
    o = (ZeroPadding2D((1, 1), data_format=IMAGE_ORDERING))(o)
    o = (
        Conv2D(
            256,
    ...
    o = (UpSampling2D((2, 2), data_format=IMAGE_ORDERING))(o)
    o = np.concatenate([o, f2], axis=MERGE_AXIS)
    o = (ZeroPadding2D((1, 1), data_format=IMAGE_ORDERING))(o)
    o = (
        Conv2D(
            128,
    ...
    if l1_skip_conn:
        o = np.concatenate([o, f1], axis=MERGE_AXIS)
...
def unet(
    n_classes, input_height=416, input_width=608, encoder_level=3, channels=3
):
...
def mobilenet_unet(
    n_classes, input_height=224, input_width=224, encoder_level=3, channels=3
):
